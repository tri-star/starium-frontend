<template>
  <v-sheet :data-test="`information-item-${information.id}`" class="information-item">
    ...
    <div data-test="information-message" class="information-message">{{ information.message }}</div>
  </v-sheet>
</template>
...
const options: ThisTypedComponentOptionsWithRecordProps<Vue, Data, Method, Computed, Props> = {
  props: {
    information: {
      type: Object as Vue.PropType<Information>,required: true
      }
  },

  computed: {
    formattedDate() {
      return dayjs(this.information.date).format('YYYY-MM-DD HH:mm:ss Z')
    }
  }
}
